...
"""
Return the path to the GPU version of libxc.

A local path configured via [`set_gpu_libxc_path!`](@ref) takes precedence.
If no local path is configured, fall back to the library provided by
[`Libxc_GPU_jll`](@ref).
"""
gpu_libxc_path() = @load_preference("libxc_gpu_path",
                                    Libxc_GPU_jll.is_available() ? Libxc_GPU_jll.libxc : nothing)

"""Is the CUDA version of libxc available on this platform"""
has_cuda() = !isnothing(gpu_libxc_path())

"""
    set_gpu_libxc_path!(path::Union{AbstractString,Nothing})

Configure a local GPU version of libxc to use instead of the one provided by
[`Libxc_GPU_jll`](@ref). The `path` must point to a shared library compatible
with the CUDA version in use.

Set `path` to `nothing` to remove a previously configured local path and fall
back to [`Libxc_GPU_jll`](@ref).

The preference is stored in `LocalPreferences.toml` and takes effect after
restarting Julia.
"""
function set_gpu_libxc_path!(path::Union{AbstractString,Nothing})
    if isnothing(path)
        @delete_preferences!("libxc_gpu_path")
    else
        isfile(path) || error("Path $path does not exist.")
        @set_preferences!("libxc_gpu_path" => path)
    end
end
...
export set_gpu_libxc_path!, gpu_libxc_path
